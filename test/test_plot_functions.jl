@testset "WF diagnostic plot functions" begin
    ...
    marginal_CI = DualOptimalFiltering_proof.compute_marginal_CI(α, Λ_of_t, wms_of_t; mass = 0.95)
    @test_nowarn DualOptimalFiltering_proof.plot_marginal_posterior_credible_interval_and_data_given_marginalCI_with_hidden_state(α, Λ_of_t, wms_of_t, data, wfchain, marginal_CI)
...
@testset "CIR diagnostic plot functions" begin
    ...
    @test_nowarn DualOptimalFiltering_proof.plot_data_and_posterior_distribution(δ, θ_of_t_CIR, Λ_of_t_CIR, wms_of_t_CIR, data_CIR, X_CIR)
