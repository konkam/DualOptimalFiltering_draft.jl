...
@testset "test high level statistical distances" begin
    ...
    res = DualOptimalFiltering.quadgk_singularities(sin, [0.5], 0, pi)
    @test res[1] ≈ 2 atol=res[2]
    res = DualOptimalFiltering.quadgk_singularities(sin, [0.5, 0.5, 0.7, 0.9], 0, pi)
    @test res[1] ≈ 2 atol=res[2]
    res = DualOptimalFiltering.quadgk_singularities(sin, [0.5, 0.5, 0.7, 0.9, 50], 0, pi)
    @test res[1] ≈ 2 atol=res[2]
end;
